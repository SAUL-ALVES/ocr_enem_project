...
def detectar_codigo_por_bolhas(roi_gabarito):
    ...
    img_thresh = cv2.adaptiveThreshold(
        img_blur, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 15, 4
    )
    ...
    for idx, coluna_cnts in enumerate(colunas[:2]):
        ...
        if maior_media_encontrada > LIMIAR_DE_PREENCHIMENTO:
            ...
            if len(coluna_cnts) > 1:
                altura_celula = (y_ultima_encontrada - y_primeira_encontrada) / (
                    len(coluna_cnts) - 1
                )
            else:
                altura_celula = 0
            ...
            if altura_celula > 0:
                indice_relativo = round(
                    (y_marcado - y_primeira_encontrada) / altura_celula
                )
            else:
                indice_relativo = 0
            ...
            cv2.circle(
                roi_gabarito, (x + w // 2, y + h // 2), int(w * 0.6), (0, 255, 0), 2
            )
            print(
                f"Análise da Coluna {idx+1}: Bolha MARCADA encontrada com intensidade {maior_media_encontrada:.2f}, dígito {digito_marcado}\n"
            )
        else:
            codigo_aluno += "?"
            print(
                f"Análise da Coluna {idx+1}: Nenhuma bolha marcada (maior intensidade foi {maior_media_encontrada:.2f}, abaixo de {LIMIAR_DE_PREENCHIMENTO})\n"
            )
...
def extrair_codigo_aluno_automatico(caminho_imagem):
    try:
        ...
        print(
            f"Redimensionando imagem de {largura}x{altura} para {LARGURA_PADRAO}x{nova_altura}"
        )
        imagem_original = cv2.resize(
            imagem_original, (LARGURA_PADRAO, nova_altura), interpolation=cv2.INTER_AREA
        )
        ...
        dados = pytesseract.image_to_data(
            imagem_binaria, output_type=pytesseract.Output.DICT, lang="por"
        )
        ...
        for i in range(len(dados["level"])):
            ...
            if conf > 40 and re.search(r"assinatur", palavra):
                ...
                print(
                    f"✅ Âncora 'Assinatura' encontrada (variação: '{palavra}') em: x={x_ancora}, y={y_ancora}"
                )
                break
        ...
        if anchor_box is None:
            ...
            for i, palavra in enumerate(dados["text"]):
                if "simulado" in palavra.lower():
                    ...
                    anchor_box = (x_ancora, y_ancora)
                    print(
                        f"⚠️ Usando fallback com 'Simulado' em x={x_ancora}, y={y_ancora}"
                    )
                    break
        ...
        print("\n--- Depuração da ROI ---")
        print(
            f"Dimensões da Imagem (Pós-redimensionamento): Largura={w_img}, Altura={h_img}"
        )
        print(
            f"ROI Calculada: x=[{x_roi_start}:{x_roi_end}], y=[{y_roi_start}:{y_roi_end}]"
        )

        if x_roi_start < 0 or y_roi_start < 0 or x_roi_end > w_img or y_roi_end > h_img:
            print(
                "AVISO: Coordenadas da ROI fora dos limites. Ajustando para caber na imagem."
            )
            ...
            print(
                f"▶️ ROI Ajustada: x=[{x_roi_start}:{x_roi_end}], y=[{y_roi_start}:{y_roi_end}]"
            )
        ...
        imagem_original[y_roi_start:y_roi_end, x_roi_start:x_roi_end] = (
            roi_com_deteccoes
        )

        cv2.rectangle(
            imagem_original,
            (x_roi_start, y_roi_start),
            (x_roi_end, y_roi_end),
            (0, 0, 255),
            2,
        )
        ...
        print("\n--- RESULTADO ---")
        print(
            f"Código do Aluno por Bolhas: {codigo_limpo if codigo_limpo else 'Nenhum código detectado.'}"
        )
