...
def detectar_codigo_por_bolhas(roi_gabarito):
    """
    Versão final com limiar de preenchimento para lidar com folhas não marcadas.
    """
    ...
    for idx, coluna_cnts in enumerate(colunas[:2]):
        ...
        if maior_media_encontrada > LIMIAR_DE_PREENCHIMENTO:
            ...
            altura_celula = (
                (y_ultima_encontrada - y_primeira_encontrada) / (len(coluna_cnts) - 1)
                if len(coluna_cnts) > 1
                else 0
            )

            indice_relativo = (
                round((y_marcado - y_primeira_encontrada) / altura_celula)
                if altura_celula > 0
                else 0
            )

            digito_real_da_primeira = 1
            ...
            print(
                f"Análise da Coluna {idx+1}: Nenhuma bolha marcada encontrada (maior intensidade foi {maior_media_encontrada:.2f}, abaixo do limiar de {LIMIAR_DE_PREENCHIMENTO})\n"
            )
...
def extrair_codigo_aluno_automatico(caminho_imagem):
    try:
        ...
        if anchor_box is None:
            print(
                "ERRO: Âncora 'Assinatura' não encontrada. Usando fallback aproximado..."
            )
            for i, palavra in enumerate(dados["text"]):
                if "simulado" in palavra.lower():
                    ...
                    # ajusta ROI relativa a "Simulado"
                    x_ancora = x_ancora - 280  # leva mais para a esquerda
                    y_ancora = y_ancora + 60  # desloca para baixo
        ...
        if x_roi_start < 0 or y_roi_start < 0 or x_roi_end > w_img or y_roi_end > h_img:
            print(
                "ERRO CRÍTICO: As coordenadas da ROI estão fora dos limites da imagem."
            )
            print(
                "▶️ Ação: Ajuste os valores de offset_y, offset_x, roi_height e roi_width."
            )
            return None
